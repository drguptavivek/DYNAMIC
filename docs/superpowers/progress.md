...
| Promotion failure propagation                | `apps/api/src/services/eventProcessor.ts`, `smoke.integration.ts`                                          | ✅ Done    | WQ/PEF/UF/POF/BAF/NFF/CDF promotion errors now rethrow; integration test confirms failed promotion is not accepted |
| Birth Outcome concept boundary               | `docs/superpowers/DataModelConcepts.md`                                                                    | ✅ Done    | documents per-outcome `birth_outcome_id`, multiple outcomes per pregnancy, and child follow-up records without HH roster additions |
...
| Failed promotion rollback                    | `apps/api/src/routes/sync.ts`, `smoke.integration.ts`                                                      | ✅ Done    | failed form promotion removes the inserted response row so the client can retry instead of seeing a duplicate |
| Persistent integration DB cleanup            | `admin-users.integration.ts`, `masters.integration.ts`, `admin-workflows.integration.ts`                   | ✅ Done    | random test rows are removed in `finally`; seed household status restored after correction tests |
