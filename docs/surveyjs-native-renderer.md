...
| Camera | `expo-image-picker` | generic camera fields retain local metadata; PEF Q11 ultrasound reports and Q47 ANC card use the durable attachment outbox |
| File picker | `expo-document-picker` | local asset metadata/URI pending attachment persistence and sync |

Permission denial is a visible control status, not a renderer fallback. A production attachment question must provide durable app-owned storage, validation, an attachment outbox, authenticated server acceptance, and retry semantics; PEF Q11 ultrasound reports and the single-image Q47 ANC card control implement that path.
...
- Generic camera/file selection remains metadata-only. PEF Q11 and Q47 use dedicated durable image workflows; broader reusable attachment support and at-rest application-layer encryption remain separate work.
